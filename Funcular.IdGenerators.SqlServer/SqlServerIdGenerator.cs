...
public static class SqlServerIdGenerator
{
    ...
    /// <summary>
    /// Returns a new Base 36 Id with 11 timestamp characters, 4 server hash characters,
    /// 5 random characters, and no delimiter, with the timestamp component set to <paramref name="creationTimestamp"/>. 
    /// </summary>
    /// <returns></returns>
    [return: SqlFacet(IsFixedLength = false, IsNullable = false, MaxSize = 50)]
    [SqlFunction(IsDeterministic = false, DataAccess = DataAccessKind.None)]
    public static SqlChars NewBase36IdFromTimestamp(DateTime creationTimestamp)
    {
        // Ensure a generator exists with the specific desired component lengths:
        // Cache it:
        var generator = InitializeGenerator(generatorKey: DEFAULT_GENERATOR_KEY, 11, 4, 5, null, null, null);
    ...
    /// <summary>
    /// Returns a new Base 36 Id with 10 timestamp characters, 2 server hash characters,
    /// 4 random characters, and no delimiter. 
    /// </summary>
    /// <returns></returns>
    [return: SqlFacet(IsFixedLength = false, IsNullable = false, MaxSize = 50)]
    [SqlFunction(IsDeterministic = false, DataAccess = DataAccessKind.None)]
    public static SqlChars NewBase36Id16()
    {
        var generatorKey = "11-2-3-null-null";
        // Ensure a generator exists with the specific desired component lengths:
        // Cache it:
        var generator = InitializeGenerator(generatorKey: generatorKey, 11, 2, 3, null, null, null);
        return new SqlChars(generator.NewId().ToCharArray());
    }

    /// <summary>
    /// Returns a new Base 36 Id with 11 timestamp characters, 2 server hash characters,
    /// 4 random characters, and no delimiter. 
    /// </summary>
    /// <returns></returns>
    [return: SqlFacet(IsFixedLength = false, IsNullable = false, MaxSize = 50)]
    [SqlFunction(IsDeterministic = false, DataAccess = DataAccessKind.None)]
    public static SqlChars NewBase36Id16FromTimestamp(DateTime creationTimestamp)
    {
        var generatorKey = "11-2-3-null-null";
        // Ensure a generator exists with the specific desired component lengths:
        // Cache it:
        var generator = InitializeGenerator(generatorKey: generatorKey, 11, 2, 3, null, null, null);
        return new SqlChars(generator.NewId(creationTimestamp.ToUniversalTime()).ToCharArray());
    }
    ...
    public static SqlChars CustomBase36Id(int numTimestampCharacters = 11, int numServerCharacters = 4,
        ...
        int numDelimiters = 0, DateTime? creationTimestamp = null)
    {
        ...
        var generator = InitializeGenerator(
                ...
                delimiter: delimiter,
                positions.ToArray());
        return new SqlChars(generator.NewId(delimiter?.Length > 0, creationTimestamp));
    }
